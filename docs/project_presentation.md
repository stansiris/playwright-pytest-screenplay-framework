...
- Playwright auto-waiting via locator assertions exposed through `Ensure`
...
Step files are adapter-only. Any repeated behavior becomes a Task/Question/Consequence/Interaction and is reused.
...
Activity (ABC)
  ...
  +-- Interaction
        ...
        + examples: Click, Fill, PressKey, NavigateTo, RefreshPage

Consequence (Activity)
  + perform_as(actor) in concrete consequences
  + examples: Ensure(...).to_be_visible(), Ensure(...).to_have_text(...)

Question (ABC)
  + answered_by(actor)
  |
  +-- concrete questions
        + examples: OnInventoryPage, OnLoginPage, TextOf, TotalsMatchComputedSum

Actor
  + can(ability)
  + ability_to(ability_class)
  + attempts_to(*activities: Task|Consequence)
  + asks_for(question)
