...
def test_login_page_ui_elements(customer: Actor) -> None:
    ...
    customer.attempts_to(Ensure.that(LoginPage.LOGIN_LOGO).to_be_visible())
    customer.attempts_to(Ensure.that(LoginPage.LOGIN_USERNAME).to_be_visible())
    customer.attempts_to(Ensure.that(LoginPage.LOGIN_PASSWORD).to_be_visible())
...
def test_inventory_page_ui_elements(customer: Actor) -> None:
    ...
    customer.attempts_to(Ensure.that(AppShell.PAGE_TITLE).to_have_text("Products"))
    customer.attempts_to(Ensure.that(InventoryPage.INVENTORY_SORT).to_be_visible())
    customer.attempts_to(Ensure.that(AppShell.SHOPPING_CART_LINK).to_be_visible())
    customer.attempts_to(Ensure.that(InventoryPage.INVENTORY_ITEM_NAMES).to_have_count(6))
    customer.attempts_to(Ensure.that(InventoryPage.INVENTORY_ITEM_PRICES).to_have_count(6))
...
def test_product_details_page_ui_elements(customer: Actor) -> None:
    ...
    customer.attempts_to(Ensure.that(ProductDetailsPage.PRODUCT_DETAILS_IMAGE).to_be_visible())
    customer.attempts_to(Ensure.that(InventoryPage.INVENTORY_ITEM_NAMES).to_have_text(PRODUCT_NAME))
    customer.attempts_to(Ensure.that(InventoryPage.INVENTORY_ITEM_PRICES).to_be_visible())
...
def test_cart_page_ui_elements(customer: Actor) -> None:
    ...
    customer.attempts_to(Ensure.that(AppShell.PAGE_TITLE).to_have_text("Your Cart"))
    customer.attempts_to(Ensure.that(CartPage.CART_ITEM_NAMES).to_have_count(1))
    customer.attempts_to(Ensure.that(CartPage.CHECKOUT_BUTTON).to_be_visible())
...
def test_checkout_info_page_ui_elements(customer: Actor) -> None:
    ...
    customer.attempts_to(Ensure.that(CheckoutInfoPage.CHECKOUT_FIRST_NAME).to_be_visible())
    customer.attempts_to(Ensure.that(CheckoutInfoPage.CHECKOUT_LAST_NAME).to_be_visible())
    customer.attempts_to(Ensure.that(CheckoutInfoPage.CHECKOUT_POSTAL_CODE).to_be_visible())
    customer.attempts_to(Ensure.that(CheckoutInfoPage.CHECKOUT_CONTINUE).to_be_visible())
    customer.attempts_to(Ensure.that(CheckoutInfoPage.CHECKOUT_INFO_CANCEL_BUTTON).to_be_visible())
...
def test_checkout_overview_page_ui_elements(customer: Actor) -> None:
    ...
    customer.attempts_to(Ensure.that(AppShell.PAGE_TITLE).to_have_text("Checkout: Overview"))
    customer.attempts_to(
        Ensure.that(CheckoutOverviewPage.CHECKOUT_OVERVIEW_ITEM_NAMES).to_have_count(1)
    )
    customer.attempts_to(Ensure.that(CheckoutOverviewPage.CHECKOUT_PAYMENT_INFO).to_be_visible())
    customer.attempts_to(Ensure.that(CheckoutOverviewPage.CHECKOUT_SHIPPING_INFO).to_be_visible())
    customer.attempts_to(Ensure.that(CheckoutOverviewPage.CHECKOUT_SUBTOTAL).to_be_visible())
    customer.attempts_to(Ensure.that(CheckoutOverviewPage.CHECKOUT_TAX).to_be_visible())
    customer.attempts_to(Ensure.that(CheckoutOverviewPage.CHECKOUT_TOTAL).to_be_visible())
    customer.attempts_to(Ensure.that(CheckoutOverviewPage.CHECKOUT_FINISH).to_be_visible())
...
def test_checkout_complete_page_ui_elements(customer: Actor) -> None:
    ...
    customer.attempts_to(Ensure.that(CheckoutCompletePage.CHECKOUT_COMPLETE_TEXT).to_be_visible())
    customer.attempts_to(
